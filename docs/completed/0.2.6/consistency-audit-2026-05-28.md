...
0.2.6 当前已进入实现窗口：主方向是数据安全与长期数据承诺，重点是复杂本地数据的备份契约、导入预览、完整性检查和恢复预检。
...
- 备份状态边界：用户完成备份后，设置与数据界面不应继续笼统显示未备份；失败、取消、校验失败和无法确认必须有不同状态。
- 隐私边界：本地优先，无账号、无云端、无外部分析。
- 训练数据边界：当前代码未发现真实 training stores，0.2.6 只做条件覆盖，不凭规划文档虚构。

## 已修正

- `plan-0.2.6.md`：状态已同步为开发中。
...
- `user-feedback-2026-05-28.md`：记录全部导出、修复入口可发现、健康问题具体定位，以及已备份后仍提示未备份的状态反馈。

当前已完成刀 1 只读审计、刀 2 当前风险矩阵、刀 3 当前 integrity 矩阵、刀 4 当前只读预检矩阵和刀 5 当前 CSV 导出矩阵；实现窗口正在推进时仍需按最新代码复核。若 0.2.2 - 0.2.5 数据尚未真实落地，应按真实代码状态缩小 0.2.6 范围。
