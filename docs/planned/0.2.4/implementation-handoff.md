...
> 本文是 0.2.4 进入实现窗口前的短交接单。它不替代刀 1 - 刀 7，也不声明当前代码已经实现这些能力。
...
0.2.4 可以作为后续实现草案，但当前短期实现线仍是 0.2.2 / 0.2.3。若切入 0.2.4，实现窗口应先重新检查当前代码、dirty worktree、schema / migration 管线和 0.2.3 复盘入口实际形态。
