...
- snapshots。
- 0.2.2 成人行为事件。
- 0.2.4 training goals，仅在真实 store / 类型已落地后纳入。
- 0.2.4 goal check-ins，仅在真实 store / 类型已落地后纳入。
...
- 缺三类成人行为事件：补空数组。
- 若当前契约已经包含 training goals：缺失时补空数组。
- 若当前契约已经包含 goal check-ins：缺失时补空数组。
- 缺可选 metadata：用安全默认值。
...
- 成人行为事件是否作为独立维度。
- 训练数据是否作为独立维度，仅在真实 store / 类型已落地后确认。
- 如果归入“日志”，文案必须说明包含成人行为事件；训练数据只在真实落地后加入说明。

按日期筛选：

- 成人行为事件使用 `targetDate`。
- training goals 使用 startDate / endDate 或全部保留，需要在真实落地后单独定规则。
- goal check-ins 使用 targetDate / createdAt，需要在真实落地后单独定规则。

按 tag 筛选：

- 成人行为事件如果自身有 tags，应按自身 tags 判断。
- training goals 默认不受 log tags 影响，仅在真实落地后确认。
