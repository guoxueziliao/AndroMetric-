...
- 0.2.4 实现交接摘要已补齐，进入实现前仍需核对真实代码状态和 0.2.3 复盘入口实际形态。
...
- 完成 [`0.2.4/training-data-model.md`](./0.2.4/training-data-model.md) 的字段定稿。
- 重新检查当前代码状态、dirty worktree、schema / migration 管线和导入导出入口。
- 先完成 schema / migration / import-export，再进入规则建议、轻目标和 UI。
- 每一刀都对照 [`Safety Rails`](./0.2.4/safety-rails.md) 做禁用目标、禁用文案和禁用指标检查。

## 下一步

0.2.4 当前是执行草案 / 待实现。短期实现线仍是 0.2.2：先按 [`0.2.2/knife-41.md`](./0.2.2/knife-41.md) 进入 schema / migration / domain types。

0.2.4 后续继续规划时，建议从以下问题开始：

1. 如要进入 0.2.4 实现窗口，先重新检查代码状态、dirty worktree 和 0.2.3 复盘入口实际形态。
2. 若继续规划 0.2.5，优先讨论完整训练中心、长期目标历史和更丰富关系管理是否进入后续。
3. 若实现中需要扩大 0.2.4 范围，先回到本文档重新讨论。
