        ...
        <div className="space-y-4 pt-2">
            <div className="space-y-2 pb-2 border-b border-slate-100 dark:border-slate-800">
                <div className="flex gap-2">
                    <button onClick={() => setTypeFilter('all')} className={`px-3 py-1 text-[10px] font-bold rounded-full border ${typeFilter === 'all' ? 'bg-slate-800 text-white border-slate-800' : 'bg-transparent text-slate-500'}`}>全部类型</button>
                    {(['manual', 'quick', 'auto'] as HistoryEventType[]).map(t => (
                        <button key={t} onClick={() => setTypeFilter(t)} className={`px-3 py-1 text-[10px] font-bold rounded-full flex items-center border ${typeFilter === t ? `${EVENT_TYPE_CONFIG[t].color} text-white` : 'bg-transparent text-slate-500'}`}>
                            {React.createElement(EVENT_TYPE_CONFIG[t].icon, { size: 10, className: 'mr-1' })} {EVENT_TYPE_CONFIG[t].label}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="relative border-l-2 border-slate-200 dark:border-slate-800 ml-4 space-y-6 pb-4">
                {filteredItems.map((record, i) => {
                    ...
                    return (
                        <div key={i} className="relative pl-6 animate-in slide-in-from-bottom-2 fade-in">
                            ...
                            <div className="flex flex-col gap-1">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold bg-slate-100 dark:bg-slate-800 text-slate-500`}>{catLabel}</span>
                                    </div>
                                    <span className="text-[10px] text-brand-muted font-mono">{new Date(record.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                </div>
                                {record.details && record.details.length > 0 && (
                                    <div className="mt-1 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
                                        {record.details.map((d, idx) => d ? <DiffRow key={idx} diff={d} /> : null)}
                                    </div>
                                )}
                            </div>
                        </div>
