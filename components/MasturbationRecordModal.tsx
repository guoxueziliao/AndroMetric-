...
const MasturbationRecordModal: React.FC<MasturbationRecordModalProps> = ({ isOpen, onClose, onSave, initialData, dateStr, logs = [], partners = [] }) => {
    ...
    const handleSave = () => {
        // Ensure status is marked as completed when saving from modal
        const finalData: MasturbationRecordDetails = { 
            ...data, 
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            {/* Action Area: Delete (Hover) + Badge/Arrow */}
                                            <div className="flex items-center gap-2">
                                                <button 
                                                    onClick={(e) => handleDeleteContent(item.id, e)}
                                                    className="p-1.5 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all active:scale-90"
                                                >
                                                    <Trash2 size={12}/>
                                                </button>
                                                {hasIssues ? (
