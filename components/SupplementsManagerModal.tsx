                ...
                <div className="space-y-6 animate-in slide-in-from-right duration-300">
                    <div className="space-y-4">
                        ...
                        <input 
                            ...
                            placeholder="补剂名称 (如: 锌镁片, 希爱力)"
                            value={formData.name}
                            onChange={e => setFormData({...formData, name: e.target.value})}
                        />
                        <input 
                            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 font-bold outline-none focus:border-brand-accent"
                            placeholder="默认剂量 (如: 50mg, 1片)"
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-3xl border border-slate-200 dark:border-white/5 space-y-5">
                        ...
                        {formData.cycleEnabled && (
                            <div className="space-y-5 animate-in fade-in zoom-in-95">
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">总计天数 (0 为无限制)</label>
                                    <input type="number" className="w-full bg-white dark:bg-slate-900 border border-slate-200 rounded-xl p-3 text-sm font-bold" value={formData.totalCycleDays} onChange={e => setFormData({...formData, totalCycleDays: parseInt(e.target.value)||0})} />
                                </div>
                            </div>
                        )}
                    </div>
