...
const SupplementPillbox: React.FC<SupplementPillboxProps> = ({ activeSupplements, intakes, onChange }) => {
    ...
    const togglePill = (id: string) => {
        const isTaken = intakes.some(i => i.supplementId === id);
        if (isTaken) {
            onChange(intakes.filter(i => i.supplementId !== id));
        } else {
            onChange([...intakes, { supplementId: id, taken: true }]);
        }
    };
                ...
                {activeSupplements.map(sup => {
                    ...
                    return (
                        <button
                            key={sup.id}
                            type="button"
                            onClick={() => togglePill(sup.id)}
                            className={`relative p-4 rounded-3xl border-2 transition-all flex flex-col items-center gap-2 group active:scale-95 ${
                                isTaken 
                                ? `bg-white dark:bg-slate-900 shadow-md` 
                                : 'bg-slate-50 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800 border-dashed text-slate-400'
                            }`}
                            style={{ 
