...
export const RangeSlider = ({ value, min, max, onChange, leftLabel, rightLabel, colorClass = "accent-brand-accent" }: any) => (
    <div className="space-y-1">
        <div className="flex justify-between text-xs text-brand-muted dark:text-slate-400 px-1">
            <span>{leftLabel}</span>
    ...
    <div className="flex justify-between gap-1">
        {options.map((opt) => {
            ...
            return (
                ...
                >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-2xl shadow-sm border-2 ${isSelected ? `border-white dark:border-slate-800 shadow-md ${opt.color} dark:bg-opacity-80` : 'bg-slate-100 dark:bg-slate-800 border-transparent'}`}>
                        {opt.emoji}
                    </div>
...
export const MOOD_FACES: { val: Mood, label: string, emoji: string, color: string }[] = [
    { val: 'excited', label: '兴奋', emoji: '🤩', color: 'bg-pink-100 dark:bg-pink-900/50' },
    { val: 'happy', label: '开心', emoji: '😆', color: 'bg-green-100 dark:bg-green-900/50' },
    { val: 'neutral', label: '平淡', emoji: '😐', color: 'bg-slate-100 dark:bg-slate-700' },
    { val: 'anxious', label: '焦虑', emoji: '😰', color: 'bg-orange-100 dark:bg-orange-900/50' },
    { val: 'sad', label: '低落', emoji: '😭', color: 'bg-blue-100 dark:bg-blue-900/50' },
    { val: 'angry', label: '生气', emoji: '😡', color: 'bg-red-100 dark:bg-red-900/50' },
];

export const STRESS_FACES: { val: StressLevel, label: string, emoji: string, color: string }[] = [
    { val: 1, label: '放松', emoji: '😌', color: 'bg-green-100 dark:bg-green-900/50' },
    { val: 2, label: '还好', emoji: '🙂', color: 'bg-lime-100 dark:bg-lime-900/50' },
    { val: 3, label: '一般', emoji: '😐', color: 'bg-yellow-100 dark:bg-yellow-900/50' },
    { val: 4, label: '压力', emoji: '😓', color: 'bg-orange-100 dark:bg-orange-900/50' },
    { val: 5, label: '崩溃', emoji: '🤯', color: 'bg-red-100 dark:bg-red-900/50' },
];
