...
interface FABProps {
  ...
  isSexOngoing?: boolean; // 新增
}

const FAB: React.FC<FABProps> = ({ onSleep, onSex, onMasturbation, onExercise, onNap, onAlcohol, isSleepPending, isExerciseOngoing, isNapOngoing, isMbOngoing, isAlcoholOngoing, isSexOngoing }) => {
      ...
      <div className="fixed bottom-24 right-6 flex flex-col items-center z-50 pointer-events-none">
        <div className={`flex flex-col items-center mb-2 pointer-events-auto transition-all duration-200 origin-bottom ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-50 translate-y-10 pointer-events-none'}`}>
          ...
          <button
            ...
            className={`${subButtonClass} ${isSexOngoing ? 'bg-pink-500 text-white animate-pulse' : 'bg-pink-100 hover:bg-pink-200'} focus:ring-pink-300`}
            aria-label="记录性爱"
          >
            <Users size={20} className={isSexOngoing ? "text-white" : "text-pink-600"} />
          </button>
