        ...
        <form onSubmit={(e) => { e.preventDefault(); handleReview(); }} className="space-y-6 pb-8 relative">
                        ...
                        <div className="space-y-6 animate-in fade-in">
                                ...
                                {log.alcoholRecord && log.alcoholRecord.totalGrams > 0 ? (
                                    <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/30 p-3 rounded-lg flex justify-between items-center cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors" onClick={() => setIsAlcoholModalOpen(true)}>
                                        <div className="flex items-center">
                                            <div className="p-2 bg-amber-100 dark:bg-amber-800 rounded-full mr-3 text-amber-600 dark:text-amber-200"><Beer size={20} /></div>
                                            <div>
                                                <div className="font-bold text-amber-800 dark:text-amber-200 text-sm">{log.alcoholRecord.totalGrams}g 纯酒精</div>
                                                <div className="text-xs text-amber-600/70 dark:text-amber-400 flex gap-2">
                                                    {log.alcoholRecord.time && <span className="font-mono">{log.alcoholRecord.time}</span>}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-4 gap-1">
                                    {PORN_OPTS.map(opt => (
                                        <button key={opt.value} onClick={() => handleChange('pornConsumption', opt.value)} className={`p-1.5 rounded border text-[10px] text-center ${log.pornConsumption === opt.value ? 'bg-purple-100 dark:bg-purple-900 text-purple-700 border-purple-300' : 'bg-white dark:bg-slate-800 border-slate-200'}`}>{opt.label}</button>
                                    ))}
                                </div>
                            </div>
            ...
            <div className="bg-brand-secondary dark:bg-slate-900 p-4 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                        {EVENT_PRESETS.map(evt => (
                            <button key={evt} type="button" onClick={() => toggleEvent(evt)} className={`px-2.5 py-1 text-xs rounded-full border transition-all ${log.dailyEvents?.includes(evt) ? 'bg-slate-700 text-white border-slate-700 dark:bg-slate-200 dark:text-slate-900' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'}`}>{evt}</button>
                        ))}
                    </div>
                    ...
                    {log.dailyEvents && log.dailyEvents.filter(e => !EVENT_PRESETS.includes(e)).length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                            {log.dailyEvents.filter(e => !EVENT_PRESETS.includes(e)).map(e => (
                                <span key={e} className="px-2 py-1 text-xs rounded-full bg-slate-700 text-white flex items-center">{e} <button type="button" onClick={() => toggleEvent(e)} className="ml-1"><X size={10}/></button></span>
                            ))}
                        </div>
                    )}
                </div>
                ...
                <h3 className="text-sm font-bold text-brand-muted uppercase mb-3 flex items-center"><Tag size={16} className="mr-2"/>标签 & 备注</h3>
                 ...
                 <textarea value={log.notes || ''} onChange={(e) => handleChange('notes', e.target.value)} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg p-3 text-sm focus:ring-brand-accent focus:border-brand-accent text-brand-text dark:text-slate-200 min-h-[80px]" placeholder="今天感觉如何？写点什么吧..."/>
            </div>

            <GlobalTimeline log={log} />

            <div className="flex gap-3">
                 <button type="button" onClick={handleSaveDraft} className="flex-1 py-4 font-semibold text-brand-accent bg-white dark:bg-slate-800 border border-brand-accent dark:border-slate-600 rounded-lg shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex justify-center items-center"><Save className="mr-2" size={18}/> 保存草稿</button>
                <button type="submit" className="flex-[2] py-4 text-lg font-semibold text-white bg-brand-accent rounded-lg shadow-md hover:bg-brand-accent-hover transition-transform active:scale-[0.99] flex justify-center items-center"><CheckSquare className="mr-2"/> 完成日记</button>
            </div>
