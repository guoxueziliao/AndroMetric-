...
import { 
    LogEntry, PartnerProfile, AlcoholRecord, MasturbationRecordDetails, SupplementIntake
} from '../types';
...
const LogForm: React.FC<LogFormProps> = ({ onSave, existingLog, logDate, onDirtyStateChange, logs, partners }) => {
    ...
    const handleCreateEventTag = () => {
        ...
        if (!current.includes(tag)) setField('dailyEvents', [...current, tag]);
        setEventSearch('');
    };
    ...
    const handleSaveAlcohol = (r: AlcoholRecord) => {
        ...
        setLog(prev => ({ ...prev, alcoholRecords: next, alcohol: level }));
        markDirty();
    };
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-soft border border-slate-100 dark:border-white/5 overflow-hidden">
                <div className="flex bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-white/5">
                    {[{ id: 'life', label: '生活' }, { id: 'env', label: '环境' }, { id: 'health', label: '健康' }].map(t => (
                        <button 
                            key={t.id}
                        ...
                        <div className="space-y-8 animate-in fade-in duration-300">
                                    ...
                                    {log.alcoholRecords?.map(record => (
                                        <div key={record.id} className="group flex justify-between items-center bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="text-sm font-black text-slate-700 dark:text-slate-200">{record.totalGrams}克 酒精</div>
                                            </div>
                                ...
                                <div className="space-y-2">
                                    {log.caffeineRecord?.items.map(item => (
                                        <div key={item.id} className="group flex justify-between items-center bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                                            ...
                                            <div className="flex gap-1.5">
                                                <button onClick={() => handleEdit('bev', item)} className="p-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-slate-400"><Edit3 size={15}/></button>
                                                <button onClick={() => removeItem('caffeine', item.id)} className="p-2 bg-slate-50 dark:bg-slate-700 rounded-xl text-slate-400"><Trash2 size={15}/></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-white/5">
                                <div className="flex gap-3">
                                    <button onClick={() => { setEditTarget(null); setModalState(s => ({ ...s, sex: true })); }} className="flex-1 py-4 bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400 rounded-2xl font-black text-xs flex items-center justify-center gap-2 border border-pink-100 dark:border-pink-900/50">
                                        <Heart size={16} fill="currentColor" fillOpacity={0.2} /> 记录性爱
                                    </button>
                                    <button onClick={() => { setEditTarget(null); setModalState(s => ({ ...s, mb: true })); }} className="flex-1 py-4 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-2xl font-black text-xs flex items-center justify-center gap-2 border border-blue-100 dark:border-blue-900/50">
                                        <Hand size={16} /> 记录自慰
                                    </button>
                                </div>
                            </div>
