...
export const IconToggleButton = <T extends string = string>({
  options,
  selected,
  onSelect,
  renderIcon,
...
interface FaceSelectorProps<T> {
  options: { val: T; label: string; emoji: string; color: string }[];
  value: T;
  onChange: (val: T) => void;
}

export const FaceSelector = <T,>({ options, value, onChange }: FaceSelectorProps<T>) => (
